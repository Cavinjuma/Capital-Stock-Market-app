      ...
      <div className="h-80 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            {selectedStock ? (
              <Line type="monotone" dataKey={selectedStock} stroke={getStockColor(selectedStock)} strokeWidth={2} dot={{ r: 3, fill: '#818cf8', strokeWidth: 0 }} activeDot={{ r: 5, fill: '#6366f1' }} />
            ) : (
              stocks.map((stock, index) => (
                <Line key={stock} type="monotone" dataKey={stock} stroke={getStockColor(stock, index)} strokeWidth={2} dot={{ r: 3, strokeWidth: 0 }} activeDot={{ r: 5 }} />
              ))
            )}
